...
function render() {
  ...
  chart.setOption({
    ...
    yAxis: { type: 'value', name: props.yAxisName, nameTextStyle: { fontSize: 10, color: '#94a3b8' }, axisLabel: { fontSize: 10, color: '#94a3b8' }, splitLine: { lineStyle: { color: '#1e293b', type: 'dashed' } } },
    series: [{
      type: 'line', data: yData, name: props.title || '', smooth: props.smooth,
      symbol: 'none', lineStyle: { width: 2, color: props.color },
      areaStyle: props.area ? { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: props.color + '40' }, { offset: 1, color: props.color + '05' }]) } : undefined,
    }],
  })
}
